...
def lstm_multiembedding_encoder(sequence, lstm,
                 seq_lens=None, init_states=None, embedding=None, feature_embeddings={}, feature_dict={}):
    ...
    if feature_embeddings.__contains__('inpara_freq'):
        if not lstm.batch_first:
            word_inpara_freq = feature_embeddings['inpara_freq'](feature_dict['word_inpara_freq']).transpose(0, 1)
        emb_sequence = torch.cat([emb_sequence, word_inpara_freq], dim=-1)
