...
class Seq2SeqSumm(nn.Module):
    def __init__(self, vocab_size, emb_dim,
        ...
        self._attn_wb = nn.Parameter(torch.Tensor(n_hidden))
        self._attn_v = nn.Parameter(torch.Tensor(n_hidden))
        init.xavier_normal_(self._attn_wm)
        init.xavier_normal_(self._attn_wq)
        init.uniform_(self._attn_wb, - 1 / math.sqrt(n_hidden), 1 / math.sqrt(n_hidden))
        init.uniform_(self._attn_v, -INIT, INIT)
...
class AttentionalLSTMDecoder(object):
    def __init__(self, embedding, lstm, attn_w, projection, attn_b, attn_v):
        super().__init__()
    ...
    def sample_step(self, tok, states, attention, force_not_stop=False, eos=END):
        logit, states, score = self._step(tok, states, attention)
        if force_not_stop:
            logit[:, eos] = -1e8
